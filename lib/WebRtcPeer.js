...
function WebRtcPeer(mode, options, callback) {
  ...
  var offerToReceive = options.offerToReceive;
  ...
  this.generateOffer = function (callback) {
    ...
    if (mode === 'recvonly') {
      ...
      if (offerToReceive) {
        offerAudio = (typeof offerToReceive.audio === 'boolean') ?
          offerToReceive.audio : true
        offerVideo = (typeof offerToReceive.video === 'boolean') ?
          offerToReceive.video : true
      }
  ...
  if (mode !== 'recvonly' && !videoStream && !audioStream) {
    ...
    function getDisplayMedia() {
      try {
        ...
        throw new Error('Update your browser!'); // FIXME:
      }
    }
